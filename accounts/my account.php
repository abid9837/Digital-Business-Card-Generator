...
if(isset($_SESSION['user_email']) && $_SESSION['user_password']){
    ...
    $log_fetchAssoc = mysqli_fetch_assoc($log_query);
        ...
        $card_sql = "SELECT * FROM `create_cards` WHERE user_uid='$user_uid' ";
        ...
        if($card_numRows>0){
            ...
            // active order 
            $active_sql = "SELECT * FROM `create_cards` WHERE user_uid='$user_uid' AND payment_status='active' ";
            ...
            // pending order 
            $pending_sql = "SELECT * FROM `create_cards` WHERE user_uid='$user_uid' AND payment_status='on-hold' limit 1";
        ...
        <div class="account_info_div_main">
            ...
            <div id="total_order_div">
                ...
                <div class="order_list_div_main">
                    <h6>Order List : <b>Business</b> </h6>
                    <a href="./my account qr.php" style="width:152px; margin-top: 10px;" class="btn btn-primary">Show QR order</a>
                    <br>
                    <div class="table_div">
                        <table style="overflow-x: scroll;" class="table table-dark table-hover">
                            ...
                            <?php
                                // while loop 
                                $card_sql_loop = "SELECT * FROM `create_cards` WHERE user_uid='$user_uid' ";
                            ...
                            <tr>
                                ...
                                <td  style="display: flex; flex-direction: column; justify-content: center; align-items: center; height: 100%;"> 
                                    <!-- QR code -->
                                    <?php
                                        if($card_fetchAssoc_loop['QR_src']=="" && $card_fetchAssoc_loop['payment_status']=='pending' || $card_fetchAssoc_loop['payment_status']=='limit-over' ){
                                    ?>
                                            <p style="color: red;">payment not veryfied</p> <br>
                                            <a href="../templete-page/re-payment/re.payment.php?link=<?php echo $card_fetchAssoc_loop['link_number']; ?>" class="btn btn-info" style="margin-top: 10px;">Make Payment</a>
                                    <?php
                ...
                <!-- if no data found -->
                <?php }else{ ?>
                    <p>You have business card</p> 
                    <a href="./my account qr.php" style="width:152px; margin-top: 10px;" class="btn btn-primary">Show QR Code </a>
